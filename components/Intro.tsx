  ...
  return (
    <section className="mx-auto max-w-2xl px-4 sm:pb-6 lg:max-w-7xl lg:px-8">
      <div className="mb-8 flex   flex-wrap  flex-auto justify-between md:mb-16 items-center">
        <div className="mb-12  w-full md:mb-16 lg:w-2/5">
          <ImageCarousel imgUrl={data.imgUrl} time={true} />
        </div>
      ...
      <div className="flex flex-col  items-center justify-between gap-8 md:flex-row">
        <div className="flex  h-12 w-96 divide-x overflow-hidden rounded-lg border ">
          {links.map((link, id) => (
            <Link
              key={id}
              className="flex w-1/3 items-center justify-center text-base text-gray-500 transition duration-100 hover:bg-gray-100 active:bg-gray-200"
              href={link.href}
            >
              {link.name}
            </Link>
